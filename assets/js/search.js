// assets/js/search.js
// Implements trickle-down search behaviors.
// - landing (scope: all) searches searchIndex.json (site-wide index).
// - story (scope: story) filters by story id and story+chapters.
// - chapter (scope: chapter) search is local (we treat content on page).
// It listens for 'nenzee:search' CustomEvent or can be invoked directly.
...
(() => {
  ...
  const runSearch = async (q, scope) => {
    ...
    if(scope === 'all'){
      ...
      // example path: /stories/story-1/index.html
