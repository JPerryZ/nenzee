...
(() => {
  ...
  const renderResults = (results, containerSelector) => {
    ...
    cont.innerHTML = results.map((r) => 
      `<article class="card"><h3><a href="${r.url}">${r.title}</a></h3><p>${r.snippet || ''}</p><div style="font-size:0.85rem;color:rgba(255,255,255,0.6)">on <em>${r.page}</em></div></article>`
    ).join('');
  };
  ...
  const runSearch = async (q, scope) => {
    ...
    if (scope === 'all') {
      ...
      const results = idx.filter((item) => 
        matches(item.title, q) || matches(item.snippet || '', q) || matches(item.content || '', q)
      );
      ...
      let results = [];
      if (!storyId) {
        results = idx.filter((item) => 
          (item.page || '').includes('story') &&
          (matches(item.title, q) || matches(item.snippet || '', q) || matches(item.content || '', q))
        );
      } else {
        results = idx.filter((item) => 
          (item.page || '').includes(storyId) &&
          (matches(item.title, q) || matches(item.snippet || '', q) || matches(item.content || '', q))
        );
      }
      ...
      const cont = document.getElementById('chapter-search-results') ||
                   document.getElementById('story-search-results') ||
                   document.getElementById('search-results');
      if (cont) {
        cont.innerHTML = inpage.length ? inpage.map((r) =>
          `<div class="card"><h3><a href="${r.url}">${r.title}</a></h3><p>${r.snippet}</p></div>`
        ).join('') : '<div class="card"><p>No matches in this page.</p></div>';
      }
    }
  };
